...
const Series = () => {
  ...
  const filteredSeries = useMemo(() => {
    if (currentLetter) {
      return allSeries.filter((serie) =>
        serie.title.toUpperCase().startsWith(currentLetter)
      );
    }
    return allSeries;
  }, [currentLetter, allSeries]);

  useEffect(() => {
    setCurrentPage(1);
  }, [currentLetter]);

  const { totalPages, currentSeries } = useMemo(() => {
    const total = Math.ceil(filteredSeries.length / ITEMS_PER_PAGE);
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    const endIndex = startIndex + ITEMS_PER_PAGE;
    const current = filteredSeries.slice(startIndex, endIndex);
    return { totalPages: total, currentSeries: current };
  ...
  return (
    <PageLayout>
      ...
      <SerieList series={currentSeries} />
