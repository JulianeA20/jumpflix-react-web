...
const SerieList = React.memo(({ series }) => {
    const navigate = useNavigate();

    const handlePlayClick = (serie) => {
        // Determina o tipo baseado nas propriedades do objeto
        let contentType = "series";
        if (serie.type === "anime") contentType = "anime";
        if (serie.type === "dorama") contentType = "dorama";

        navigate(`/content/${contentType}/${serie.id}`);
    };
    ...
    return (
        <div>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-6 p-6">
                {series.length === 0 ? (
                    <p className="ml-6">Nenhum conteúdo encontrado.</p>
                ) : (
                    series.map((serie) => {
                        return (
...
SerieList.displayName = 'SerieList';
